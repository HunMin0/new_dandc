...
class _GroupBoardInfoState extends State<GroupBoardInfo> {
  ...
  Future<void> _updateHits() async {
    final prefs = await SharedPreferences.getInstance();
    final today = DateTime.now().toString().substring(0, 10);
    final lastViewDate = prefs.getString('article_${boardWriteId}');

    if (lastViewDate == null || lastViewDate != today) {
      // Shared Preferences에 저장된 날짜가 없거나 오늘 날짜와 다를 경우 업데이트
      ...
      // 오늘 날짜로 업데이트
      await prefs.setString('article_${boardWriteId}', today);
    } else {
      // 이미 오늘 조회했음을 알림
      // print('하루에 한 번만 조회수가 업데이트됩니다.');
    }
  }
              ...
              child: Column(
                ...
                children: [
                  ...
                  Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text("\u{1F4AC} 댓글 " + boardWriteData!.has_comments_count.toString() + '개', style: SettingStyle.SUB_TITLE_STYLE.copyWith(fontWeight: FontWeight.w500),),
                      SizedBox(
                        height: 10,
                      ),
